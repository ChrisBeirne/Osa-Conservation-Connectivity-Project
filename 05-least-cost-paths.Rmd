...
base_shp <- st_read("data/input/area_of_interest/base_aoi.shp")
...
cond_1km       <- raster("Data/output/Chapter5/Low_res_resistance_surface_final.tif")
cond_1km_terra       <- rast("Data/output/Chapter5/Low_res_resistance_surface_final.tif")
cond_1km_stars  <- read_stars("Data/output/Chapter5/Low_res_resistance_surface_final.tif")

#plot(cond_1km)

# Transform to 17N
mangrove_17N <- st_transform(mangrove_large,31971)
cond_1km_17N <- read_stars("Data/output/Chapter5/Low_res_resistance_surface_final_17N.tif")
cond_1km_utm <- raster("Data/output/Chapter5/Low_res_resistance_surface_final_NO_NA_17N.tif")
...
```{r ch5_3, eval=T, include=F}
# Import coastal protected areas
terrestrial_pa <- st_read("data/input/protected_areas/lowland_protected_areas.shp")
#plot(terrestrial_pa)
terrestrial_17N <- st_transform(terrestrial_pa, 31971)
# Simplify the dataframe 
terrestrial_17N <- terrestrial_17N[,c("NAME", "DESIG_E","DESIG_T", "GIS_ARE", "AREA_KM")]
...
saveRDS(res_terrestrial, "data/output/Chapter3/lcp_test/terrestrial_to_protected.rds")
saveRDS(res_terrestrial_shp, "data/output/Chapter3/lcp_test/terrestrial_to_protected_shp.rds")
...
res_terrestrial <- readRDS("data/output/Chapter3/lcp_test/terrestrial_to_protected.rds")
res_terrestrial_shp <- readRDS("data/output/Chapter3/lcp_test/terrestrial_to_protected_shp.rds")
...
# For each lowland protected area, take the nearest protected area... BY HIGHEST CONDUCTANCE
nearest_pa <- list()
nearest_pa_shp <- list()
i<- 1
...
saveRDS(nearest_pa, "data/output/Chapter3/lcp_results/LCP_nearest_pa_res.rds")
saveRDS(nearest_pa_shp, "data/output/Chapter3/lcp_results/LCP_nearest_pa_res_shp.rds")


```

```{r ch5_6, echo=FALSE}
res_terrestrial <- readRDS("data/output/Chapter3/lcp_results/LCP_nearest_pa_res.rds")
res_terrestrial_shp <- readRDS("data/output/Chapter3/lcp_results/LCP_nearest_pa_res_shp.rds")
```
...
library(sf)
focal_pa <- readRDS("data/input/WDPA_protected_areas/all_area_pa_shp.RDS")
...
writeRaster(pa_ras,"data/output/Chapter3/lcp_results/contiguous_pa_raster.tif", overwrite=T)
st_write(contigous_pa_area,"data/output/Chapter3/lcp_results/contigous_pa_area.shp", append=F)
...
# Add biomass
nasa_biomass <- read_stars("data/input/biomass/NASA_biomass_desnity_estimation.tif")
biomass_17N <- st_transform(nasa_biomass,31971)

write_stars(biomass_17N, "data/input/biomass/NASA_biomass_desnity_estimation_17N.tif" )
#plot(biomass_17N)

#write_stars(biomass_17N, "data/input/biomass/NASA_biomass_desnity_estimation_17N_1km.tif" )
...
gfc_2020 <- read_stars("data/input/gfc/global_forest_change_2021_v1_9.treecover2000.tif")
gfc_17N <- st_warp(gfc_2020,cond_1km_stars)
write_stars(gfc_17N,"data/input/gfc/global_forest_change_2021_v1_9.treecover2000_17N.tif")
```


```{r ch5_10, eval=T, echo=F, include=F}
# Convert to 17N

res_terrestrial_17N <- st_transform(res_terrestrial_shp, 31971) 

# Proportion protected
pa_ras <- raster("data/output/Chapter3/lcp_results/contiguous_pa_raster.tif")
plot(pa_ras)

res_terrestrial$prop_protected <- NA

# Average forest cover
gfc_17N<- read_stars("data/input/gfc/global_forest_change_2021_v1_9.treecover2000_17N.tif")
gfc_17N <- as(gfc_17N, "Raster")
res_terrestrial$prop_forest <- NA

# Cross borders?
#Import the country shape
focal_shp <- st_read("data/input/area_of_interest/all_countries_expanded.shp")
focal_shp_17N <- st_transform(focal_shp, st_crs(res_terrestrial_17N))

res_terrestrial$nCountries <- NA

high_contiguous <- st_read("data/input/area_of_interest/high_elevation_pas_contiguous.shp")
...
low_contiguous <- st_read("data/input/area_of_interest/low_elevation_pas_contiguous.shp")
...
saveRDS(res_terrestrial_final, "data/output/Chapter3/lcp_results/terrestrial_final_results.rds")
saveRDS(res_mangrove_final, "data/output/Chapter3/lcp_results/mangrove_final_results.rds")

```

```{r ch5_12, echo=F}
res_terrestrial_final <- readRDS("data/output/Chapter3/lcp_results/terrestrial_final_results.rds")
```
...
```{r, include=F, eval=F}
st_write(tmp, "data/output/Chapter7/final_ter_lcps.shp", overwrite=T, append=F)
```
...
#coastal_pa  <- st_read("data/input/protected_areas/Coastal_protected_areas.shp")
...
st_write(ter_finalists_shp_wgs, "data/output/Chapter7/PA_priority.shp", append=F)
...
```{r ch_1b, echo=F, message=F, warning=F, include=F}
library(terra)

# I made the following in qQIS using the hydbas_na_layer (scale differs depending on focal landscape)

cs_extents <- st_read("data/output/Chapter5/draft_climate_hubs.shp")
colnames(cs_extents)[colnames(cs_extents)=="ID"] <- "landscape_ID"

st_write(cs_extents, "data/output/Chapter7/draft_climate_hubs_aoi.shp", append=F)
...
write.csv(res_hubs, "data/output/Chapter7/connectivity_hubs_summaries.csv", row.names=F)
...
And summarise the characteristics of the areas:
