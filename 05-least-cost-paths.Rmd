...
st_write(s, "data/Chapter7/draft_climate_hubs.shp")
...
leaflet() %>%
  ...
  addProviderTiles("Esri.WorldImagery", group="Satellite") %>%
  #addRasterImage(tmp, group="Raster") %>% 
  addPolygons(data=s, fillColor = "yellow",
...
res_hubs <- data.frame(id=s$id, land_area=NA,lowland_pa_area=NA,upland_pa_area=NA,
...
X_sub <- components[res_terrestrial_final$ID %in% tmp_res$ID,]
