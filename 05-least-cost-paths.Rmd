...
cond_1km_stars <- read_stars("Data/Chapter5/Low_res_resistance_surface.tif")
...
set.seed(1200)
test <- st_sample(high_pa_17N, 120, type = "hexagonal")
...
cond_1km[is.na(cond_1km)==T] <- 0.0001
...
terrestrial_17N <- terrestrial_17N[,c("NAME", "DESIG_E","DESIG_T", "sovrgnt", "geounit", "GIS_ARE")]
...
# Proportion protected and contiguous protected area size
...
focal_pa_17N <- st_transform(focal_pa,crs(res_mangrove_shp))
pa_ras <- st_rasterize(focal_pa_17N, template=cond_1km_stars, align=T)
pa_ras[pa_ras>0] <- 1
pa_ras[is.na(pa_ras)==T] <- 0
plot(pa_ras)

test <- st_contour(pa_ras,contour_lines=F, breaks=c(-0.5, 0.5,1.5))
test <- test[test$GIS_AREA=="[0.5,1.5)",]
plot(st_geometry(test))

st_area(test)
test <- st_cast(test, "POLYGON")
test$UID <- 1:nrow(test)
contigous_pa_area <- test

contigous_pa_area$contig_area <- as.numeric(st_area(test)/(1000*1000))
contigous_pa_area <- contigous_pa_area[, c("contig_area")]

contigous_pa_area <- st_make_valid(contigous_pa_area)

focal_pa_17N <- st_make_valid(focal_pa_17N)


test <- st_join(focal_pa_17N, contigous_pa_area)
test$geometry <- NULL
test <- test[, c("ORIG_NAME", "contig_area")]
head(test)
test2<- test %>% group_by(ORIG_NAME) %>% summarise(sum=sum(contig_area), na.rm=T)

write_stars(pa_ras,"data/Chapter3/lcp_results/contiguous_pa_raster.tif", overwrite=T)
...
write_stars(gfc_17N,"data/spatial/gfc/global_forest_change_2020_v1_8.treecover2000_17N.tif")
```
...
gfc_17N<- read_stars("data/spatial/gfc/global_forest_change_2020_v1_8.treecover2000_17N.tif")
...
focal_shp <- st_read("data/spatial/area_of_interest/focal_countries.shp")
focal_shp_17N <- st_transform(focal_shp, st_crs(res_mangrove_17N))
...
for(i in 1:nrow(res_terrestrial))
{
   ...
   # TO deal with 0's
   tmp3 <- focal_pa[focal_pa$NAME==res_terrestrial$endID[i],]$GIS_AREA
   if(length(tmp3)>0)
      {
         res_terrestrial$end_area[i] <- focal_pa[focal_pa$NAME==res_terrestrial$endID[i],]$GIS_AREA
      }
         # Intersection countries
  tmp2 <- st_intersection(tmp_shp, focal_shp_17N)
...
tmp <- terrestrial_17N[, c("ID","NAME","DESIG_E", "geounit", "GIS_ARE")]

tmp$geometry <- NULL
head(tmp)
colnames(tmp) <- c("ID", "start_pa", "designation", "start_country", "start_area")

res_terrestrial_final <- left_join(res_terrestrial, tmp)
...
# remove parks < 1km2
res_terrestrial_final <- res_terrestrial_final[res_terrestrial_final$start_area>1,]
...
#### Number of coastal protected areas available
...
### Conclusions
As you can see form the obove map, the selected climate corridors are principally located in Panama, Costa Rica and Honduras. There is not much potential for restoration of climate corridors in El Salvador, and to a lesser extent Nicaragua, Guatamal and Belize. 
...
knitr::include_graphics("data/images/priorities.PNG")
```
