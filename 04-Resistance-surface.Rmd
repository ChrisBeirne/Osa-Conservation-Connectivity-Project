...
aoi_17N <- st_transform(base_shp,31971)
...
```{r ch4_8, echo=F, message=F, warning=F, eval=F}
...
# New method
lc <- rast("data/spatial/landcover/ESACCI-global-10m_merged.tif")
...
plot(lc)
#plot(lc_100m)
writeRaster(lc_100m, "data/spatial/landcover/ESACCI-global-100m_merged.tif", overwrite=T)
...
#plot(raster_utm)
writeRaster(elevation, "Data/Chapter6/Case_study_1_Osa/elevation.tif", overwrite=TRUE)
writeRaster(raster_utm, "Data/Chapter6/Case_study_1_Osa/elevation_utm.tif", overwrite=TRUE)
...
library(terra)
lc <- rast("data/spatial/landcover/ESACCI-global-100m_merged.tif")
...
# test <- as(just_forest,"Raster")
# test2 <- as(lc_sub,"Raster")
# leaflet() %>%
#     addProviderTiles("OpenStreetMap", group="OSM (default)") %>%
#     addProviderTiles("Esri.WorldImagery", group="Satellite") %>%
#     addRasterImage(test2, colors = cb, group="lc") %>%
#     addRasterImage(test, group="just_forest") %>%
#    
#     
#     addLegend(colors = lc_key$rgb, labels=lc_key$LCCOwnLabel)%>% 
#   addLayersControl(
#     baseGroups = c("OSM (default)", "Satellite"),
#     overlayGroups = c("lc",  "just_forest")) 
...
#  addProviderTiles("Esri.WorldImagery", group="Satellite") %>%
#  addRasterImage(biomass_sub_ras, group="Biomass") %>%
#  addRasterImage(lc_re_ras, group="Resistance") %>%
#  addLayersControl(
#    baseGroups = c("OSM (default)", "Satellite"),
#    overlayGroups = c( "Biomass","Resistance")) 
```
...
writeRaster(cost_final, "Data/Chapter5/High_res_resistance_surface_final.tif", overwrite=T)
...
writeRaster(tmp, "Data/Chapter5/High_res_resistance_surface_final_17N.tif", overwrite=T)

writeRaster(tmp2, "Data/Chapter5/High_res_resistance_surface_final_17N_NO_NA.tif", overwrite=T)

#########################################
# 1km resolution ########################
#########################################
# Make  1km version too for safety's sake

cost_final <- rast("Data/Chapter5/High_res_resistance_surface_final.tif")
cost_final_17N <- rast("Data/Chapter5/High_res_resistance_surface_final_17N.tif")
cost_final_17N_NO_NA <- rast("Data/Chapter5/High_res_resistance_surface_final_17N_NO_NA.tif")
...
writeRaster(cost_final_1km, "Data/Chapter5/Low_res_resistance_surface_final.tif", overwrite=T)

cost_final_1km_17N <- aggregate(cost_final_17N, fact=10, fun="mean", na.rm=T)
writeRaster(cost_final_1km_17N, "Data/Chapter5/Low_res_resistance_surface_final_17N.tif", overwrite=T)

cost_final_1km_17N_NO_NA <- aggregate(cost_final_17N_NO_NA, fact=10, fun="mean", na.rm=T)
writeRaster(cost_final_1km_17N_NO_NA, "Data/Chapter5/Low_res_resistance_surface_final_NO_NA_17N.tif", overwrite=T)
...
writeRaster(cost_final_200m, "Data/Chapter5/Mid_res_resistance_surface_final.tif", overwrite=T)

cost_final_200m_17N <- aggregate(cost_final_17N, fact=2, fun="mean", na.rm=T)
writeRaster(cost_final_200m_17N, "Data/Chapter5/Mid_res_resistance_surface_final_17N.tif", overwrite=T)

cost_final_200m_17N_NO_NA <- aggregate(cost_final_17N_NO_NA, fact=2, fun="mean", na.rm=T)
writeRaster(cost_final_200m_17N_NO_NA, "Data/Chapter5/Mid_res_resistance_surface_final_17N_NO_NA.tif", overwrite=T)
