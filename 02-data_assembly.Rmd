...
st_write(base_aoi, "data/spatial/area_of_interest/all_countries_expanded.shp")
...
base_terra <- vect("data/spatial/area_of_interest/base_aoi.shp", crs="epsg:4326")
plot(base_terra)
...
saveRDS(all_pa, "data/spatial/WDPA_protected_areas/all_area_pa_shp.RDS")
...
drive_upload("data/spatial/WDPA_protected_areas/all_area_pa_shp.RDS", as_id("https://drive.google.com/drive/u/0/folders/1KgNuMrKPyskUmroOBGISt3FxAsaBLmRv"))
...
plot(nasa_biomass)
writeRaster(nasa_biomass, "data/spatial/biomass/NASA_biomass_desnity_estimation.tif", overwrite=T)
...
# Run hashed files the first time only
# height_upper <- rast("data/spatial/height/global_forest_canopy_height_2005.1_upper.tif")
# height_lower <- rast("data/spatial/height/global_forest_canopy_height_2005_lower.1.tif")
# m2 <- mosaic(height_upper, height_lower)
# height <- mask(m2, base_terra)
# plot(height)
# writeRaster(height, "data/spatial/height/global_forest_canopy_height_2005.1.tif", overwrite=T)

```


```{r ch2_22}
height <- rast("data/spatial/height/global_forest_canopy_height_2005.1.tif")
plot(height)
...
```{r ch2_26}
# First time run
# ghm_raster <- rast("data/spatial/human_modification_index/gHM_2016.tif")
# ghm <- mask(ghm_raster, base_terra)
# plot(ghm)
# writeRaster(ghm, "data/spatial/human_modification_index/gHM_2016.tif", overwrite=T)
...
to_import <- list.files("data/spatial/landcover/v2_global/", full=T)
r1 <- rast(to_import[1])

dem_crop <- mask(dem_raster, base_terra)
# lcc <- mask(r1, base_terra)
# plot(lcc)
# writeRaster(lcc, "data/spatial/landcover/ESACCI-global-10m.tif", overwrite=T)

raster_files <- list.files("data/spatial/landcover/v2_global", full.names=T)
test <- terra::vrt(raster_files, "data/spatial/landcover/virtual_raster", overwrite=T) 
...
writeRaster(test, "data/spatial/landcover/test.tif")
...
```{r  ch2_27, echo=F}
lc_raster <- rast("data/spatial/landcover/ESACCI-global-10m.tif")
...
#plot(lc_raster, breaks=seq(9.5,100.5, by=10),col=lc_key$rgb[1:11], key.pos=NULL)
...
low_pa <- st_intersection(focal_pa, low_sf)
...
# Only include contiguous areas > 3km2
...
tmp <- low_contiguous[as.numeric(low_contiguous$low_area_km2)>3,]
...
# Remove the border park again
low_final <- low_final[low_final$NAME!="Corredor Fronterizo",]
...
st_write(low_focal, "data/spatial/protected_areas/lowland_protected_areas.shp", overwrite=T, append=F)

st_write(low_contiguous, "data/spatial/area_of_interest/low_elevation_pas_contiguous.shp", append=F)
...
It would be useful to know if these habitats encompass coastal locations too, so we will add a ture/false column. 
...
# Buffer the focal area they crop the coastline layer (to remove islands etc)
tmp <- st_transform(base_shp, 3160 )
tmp <- st_buffer(tmp, 500)
tmp <- st_transform(tmp, 4326)

focal_coast <- st_intersection(coast_shp, tmp)
#plot(st_geometry(focal_coast), col="red", lwd=2)
tmp <- st_transform(focal_coast, 3160)
tmp <- st_buffer(tmp, 5000)
tmp <- st_transform(tmp, 4326)
tmp <- st_union(tmp)

# Intersect the lowland protected areas with buffer
lowland_pa$coastal_habitat <- as.numeric(st_intersects(lowland_pa, tmp, sparse=F ))

# Add it to a leaflet to check
leaflet() %>% 
  addTiles() %>% 
  addPolygons(data=lowland_pa[lowland_pa$coastal_habitat==0,], weight = 2, fillColor = "red") %>% 
  addPolygons(data=lowland_pa[lowland_pa$coastal_habitat==1,], weight = 2, fillColor = "blue") 
  
st_write(lowland_pa, "data/spatial/protected_areas/lowland_protected_areas.shp", overwrite=T, append=F)
...
plot(st_geometry(lowland_pa), col="red", add=T)
...
# #plot(st_geometry(focal_shp), add=T, col="lightgrey")
...
st_write(high_focal, "data/spatial/area_of_interest/high_elevation_pas.shp", append=F)
st_write(high_contiguous, "data/spatial/area_of_interest/high_elevation_pas_contiguous.shp", append=F)
